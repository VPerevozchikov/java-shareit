...
public class ItemServiceIntegrationTest {
    ...
    void getItemsByUserIdTest() {
        ...
        Set<ItemDto> itemsDtoSet = itemService.getItemsByUserId(userOne.getId());
        ArrayList<ItemDto> itemDtoArrayList = new ArrayList<>(itemsDtoSet);

        assertThat(itemsDtoSet.size(), equalTo(2));
        assertThat(itemDtoArrayList.get(0).getId(), equalTo(itemDtoOne.getId()));
        assertThat(itemDtoArrayList.get(0).getComments().get(0).getText(), equalTo(comment.getText()));
        assertThat(itemDtoArrayList.get(0).getLastBooking().getItem().getId(), equalTo(booking.getItem().getId()));
    }
}
